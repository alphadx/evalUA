...
2. **Tarjetas de Descriptores Interactivas:** Cada descriptor se muestra como un botón de alto contraste con:
   ...
   - Al hacer clic:
     ...
     - Se guarda el puntaje temporalmente en la evaluación, preservando la observación existente del criterio si la hubiera.
     - Se muestra un **toast de confirmación** (1.8s) en la parte superior con el formato "✓ {Criterio}: Nota {n} — {Etiqueta}" (ej: "✓ Calidad del Código: Nota 5 — Aceptable"), usando fondo verde (`--color-evalUA21`).
...
5. **Feedback de Observaciones por Criterio:** Al presionar el botón "💬 Agregar observación a este criterio" se despliega un panel inline con un campo de texto. El panel incluye un botón "✓ Guardar observación" (estilo primario) que al presionarse: (a) persiste la observación en el puntaje del criterio, (b) cierra el panel, y (c) muestra un toast de confirmación (1.8s) indicando "✓ Observación guardada para {Nombre del criterio}". También se incluye un botón "Cancelar" que cierra el panel sin guardar. Las observaciones guardadas se visualizan en la vista de resultados como "Motivo de la calificación".
...
- **Propósito:** Visualización de solo lectura del estado final de una evaluación. La interfaz presenta cada criterio como una fila colapsable dentro de un patrón de acordeón single-open (solo un criterio expandido a la vez):
  ...
  - **Motivo de la calificación:** Si el evaluador agregó una observación al calificar un criterio (campo `observaciones` del `Puntaje`), esta se muestra al final del detalle expandido como un bloque destacado con borde izquierdo naranja (`--color-evalUA1`) y el encabezado "💬 Motivo de la calificación:". Esto permite al evaluado entender la justificación individual de cada nota asignada.
